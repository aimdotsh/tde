...
```
create table test.t_test( id varchar2(10),name varchar2(100));
insert into test.t_test select level,'test'||level from dual connect by level<1000000;
create table test.t_test2( id varchar2(10),name varchar2(100));
insert into test.t_test2 select level,'test'||level from dual connect by level<1000000;
...
begin
  while 1 = 1 loop
    select count(1) into v_num from test.t_test where id=v_id;
  end loop;
end;
/

update test.t_test set name='test1' where id='1';
